...
(async () => {
  ...
  // Get all projects
  let nowPlayingTvRef = db.collection('nowPlayingTv');
  const query = nowPlayingTvRef.orderBy('createdAt', 'desc');
  ...
  videos.forEach(video => {
    let reviewsRef = db.collection('nowPlayingTv').doc(video.uid).collection('reviews');
    // Make reviews
    ...
    nowPlayingTvRef.doc(video.uid).update({
